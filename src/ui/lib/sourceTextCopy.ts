import type { DocumentPayload, RenderResult, SourceLocation, SourceSelectionBlock } from "../../core/types";
...
export function originalTextReferenceForSelection({
  ...
  renderResult:
    | (Pick<RenderResult, "sourceTextBlocks"> &
        Partial<Pick<RenderResult, "sourceBlocks" | "sourceSelectionBlocks" | "headings">>)
    | null
    | undefined;
}) {
  ...
  const expanded = sourceSelectionReferenceForRange(
    range, article, document, renderResult, section,
  );
  if (expanded) return { value: expanded.value };
  const units = sourceUnitsForSelection(
    range,
    article,
    document,
    renderResult,
  ...
  const source = sourceForBlock(document, first.sourcePath);
  if (!source) return undefined;
  const text = source
    .split("\n")
    .slice(first.startLine - 1, last.endLine)
    .join("\n");
  return formatOriginalTextReference(
...
export function sourceReferenceForSelection({
  ...
  const selection = window.getSelection();
  if (!article || !selection?.rangeCount || selection.isCollapsed) return undefined;
  const expanded = sourceSelectionReferenceForRange(
    selection.getRangeAt(0), article, document, renderResult,
  );
  if (expanded) return expanded.sourceReference;
  const units = sourceUnitsForSelection(
  ...
  const first = units?.[0];
  return first
    ? formatSourceReference({ sourcePath: first.sourcePath }, document.path, first.startLine)
    : undefined;
}
...
function sourceSelectionReferenceForRange(
  ...
  const selected = candidates.slice(startIndex, endIndex + 1).map((element) =>
    selectionBlockForElement(element, candidates, blocks),
  );
  if (selected.some((block) => !block)) return undefined;
  const fragments: Array<{ path: string; startLine: number; endLine: number }> = [];
  for (const block of selected as SourceSelectionBlock[]) {
    const path = block.sourceLocation?.sourcePath ?? document.path;
    const last = fragments.at(-1);
    if (last && normalizePath(last.path) === normalizePath(path)) {
      if (block.startLine <= last.endLine) return undefined;
      last.endLine = block.endLine;
    } else {
      fragments.push({ path, startLine: block.startLine, endLine: block.endLine });
    }
  }
  const values = fragments.map((fragment) => {
    const source = sourceForBlock(document, fragment.path);
    if (!source) return undefined;
    const text = source.split("\n").slice(fragment.startLine - 1, fragment.endLine).join("\n");
    return `File: ${fragment.path}:${fragment.startLine}-${fragment.endLine}${section ? `\nSection: ${section}` : ""}\nOriginal text:\n${text}`;
  });
  if (values.some((value) => !value)) return undefined;
  return { value: values.join("\n\n"), sourceReference: `${fragments[0].path}:${fragments[0].startLine}` };
}
...
function selectionBlockForElement(
  ...
  if (element.closest(".source-block-collapsed,.admonitionblock,.admonition")) return undefined;
  const id = element.getAttribute("data-source-selection-block-id");
  if (id) return blocks.find((block) => block.id === id);
  if (!element.matches(".diagram-slot,.diagram-inline,.diagram-inline-image,.diagram-inline-diagnostic")) return undefined;
  const diagrams = candidates.filter((candidate) => candidate.matches(".diagram-slot,.diagram-inline,.diagram-inline-image,.diagram-inline-diagnostic"));
  const index = diagrams.indexOf(element);
  return index < 0 ? undefined : blocks.filter((block) => block.kind === "diagram")[index];
}
...
function sourceUnitsForSelection(
  ...
  const units = candidates.slice(startIndex, endIndex + 1).map((element) =>
    sourceUnitForElement(element, document, renderResult),
  );
  if (units.some((unit) => !unit)) return undefined;
  const resolved = units as SourceUnit[];
...
function sourceUnitForElement(
  ...
  if (element.matches("p")) {
    ...
    const block = renderResult?.sourceTextBlocks?.find((item) => item.id === id);
    ...
    return { element, sourcePath, startLine: block.startLine, endLine: block.endLine };
  }
  if (!element.matches(".source-block-frame pre") || element.closest(".source-block-collapsed")) {
    return undefined;
  }
  ...
  return { element, sourcePath, startLine, endLine: endIndex + 1 };
}
...
function codeBlockEndIndex(lines: string[], start: number, format: DocumentPayload["format"]) {
  if (format === "markdown") {
    ...
    for (let index = start + 1; index < lines.length; index += 1) {
      if (new RegExp(`^\\s*${marker}{${length},}\\s*$`).test(lines[index])) return index;
    }
    return undefined;
  }
...
function formatOriginalTextReference(
  ...
  const lineRange = endLine === line ? `${line}` : `${line}-${endLine}`;
  return {
    value: `File: ${location?.sourcePath ?? documentPath}:${lineRange}${section ? `\nSection: ${section}` : ""}\nOriginal text:\n${text}`,
  };
}
...
function formatSourceReference(
  ...
  return `${location?.sourcePath ?? documentPath}:${line}`;
}

function sourceTextBlockElement(node: Node) {
  const element =
    node.nodeType === Node.ELEMENT_NODE
      ? (node as HTMLElement)
      : node.parentElement;
  return element?.closest<HTMLElement>("[data-source-text-block-id]") ?? null;
}

function sourcePreElement(node: Node) {
  const element = node.nodeType === Node.ELEMENT_NODE ? (node as HTMLElement) : node.parentElement;
...
export function sourceTextBlockForSelection(args: Parameters<typeof originalTextReferenceForSelection>[0]) {
