...
import { runMockAgentTurn } from "./agentTurn";
...
export class MockAgentFacade {
  ...
  async sendAgentTurn(input: AgentTurnInput): Promise<AgentTurnOutcome> {
    return runMockAgentTurn(input, {
      getSession: (clientSessionId) => this.agentSessions.get(clientSessionId),
      nextTimestamp: () => this.nextAgentSessionTimestamp(),
      recordTurn: (turnInput, answer, status, steeringMessages) =>
      ...
      requireRecord: (clientSessionId) =>
        this.requireAgentSessionRecord(clientSessionId),
    });
  }
