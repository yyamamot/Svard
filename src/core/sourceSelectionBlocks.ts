...
export function extractSourceSelectionBlocks(
  ...
  const ranges =
    format === "markdown" ? markdownRanges(lines) : asciidocRanges(lines);
...
function markdownRanges(lines: string[]): LineRange[] {
  ...
  for (let index = 0; index < lines.length; ) {
    ...
    if (/^#{1,6}\s+/.test(line)) {
      ranges.push({ kind: "heading", start: index, end: index });
      index += 1;
      continue;
    }
    ...
    if (fence) {
      ...
      while (
        ...
        !new RegExp(`^\\s*${marker}{${length},}\\s*$`).test(lines[end])
      )
        end += 1;
      if (end >= lines.length) {
        index += 1;
      ...
      ranges.push({
        kind: diagramTypes.has(fence[2].toLowerCase()) ? "diagram" : "code",
        start: index,
        end,
      });
      index = end + 1;
    ...
    if (isMarkdownTable(lines, index)) {
      ...
      ranges.push({ kind: "table", start: index, end: end - 1 });
      index = end;
      continue;
    }
    if (isSimpleMarkdownList(line)) {
      ...
      while (
        ...
        (isSimpleMarkdownList(lines[end]) || !lines[end].trim())
      )
        end += 1;
      if (
        lines
          .slice(index, end)
          .some((item) => /^\s{2,}(?:[-*+] |\d+\. )/.test(item))
      ) {
        index = end;
        continue;
      }
      ranges.push({
        kind: "list",
        start: index,
        end: lastContentLine(lines, index, end),
      });
      index = end;
      continue;
    }
    ...
    while (
      ...
      !isSimpleMarkdownList(lines[index]) &&
      !isMarkdownTable(lines, index)
    )
      index += 1;
    if (index > start)
      ranges.push({ kind: "paragraph", start, end: index - 1 });
    else index += 1;
  }
  return ranges;
}
...
function asciidocRanges(lines: string[]): LineRange[] {
  ...
  for (let index = 0; index < lines.length; ) {
    ...
    if (trimmed === "|===") {
      ...
      if (end < lines.length) {
        ranges.push({ kind: "table", start: index, end });
        index = end + 1;
        continue;
      }
    }
    ...
    if (isSimpleAsciiDocList(trimmed)) {
      ...
      while (
        ...
        (isSimpleAsciiDocList(lines[end].trim()) || !lines[end].trim())
      )
        end += 1;
      if (lines.slice(index, end).some((item) => /^\s+[*-]\s+/.test(item))) {
        index = end;
        continue;
      }
      ranges.push({
    ...
    while (
      ...
      !isSimpleAsciiDocList(lines[index].trim()) &&
      lines[index].trim() !== "|==="
    )
      index += 1;
    if (index > start)
...
function locationForRange(
  ...
  if (!origins)
    return { startLine: start + 1, endLine: end + 1, sourcePath: undefined };
  const range = origins.slice(start, end + 1);
  const first = range[0];
  if (
    !first ||
    !range.every(
      (origin, offset) =>
        origin.sourcePath === first.sourcePath &&
        origin.line === first.line + offset,
    )
  )
    return undefined;
  return {
    startLine: first.line,
    endLine: range.at(-1)!.line,
...
function lastContentLine(lines: string[], start: number, end: number) {
  let index = end - 1;
  while (index > start && !lines[index].trim()) index -= 1;
  return index;
}
